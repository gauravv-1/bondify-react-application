...
const BottomNavbar = ({ activeSection, setActiveSection }) => {
  ...
  const menuItems = [
    { name: "Home", icon: <Home sx={{ fontSize: 25, color: activeSection === "Home" ? 'orange' : 'white' }} /> },
    { name: "Post", icon: <AddCircleOutlineIcon sx={{ fontSize: 25, color: activeSection === "Post" ? 'orange' : 'white' }} /> },
    { name: "Messages", icon: <Message sx={{ fontSize: 25, color: activeSection === "Messages" ? 'orange' : 'white' }} /> },
  ...
  return (
    <div className="md:hidden fixed bottom-0 w-full bg-gray-950 flex justify-around py-4 border-t-[0.5px]">
      {menuItems?.map((item) => (
        <button
          key={item?.name}
